...
- `ports`: for every optical or electrical port of the component, please give the following information:
  ...
  - `modes`: (optional) if the optical modes differ between ports, please add the modes relevant to the component port here instead of specifying the modes for the component in general. Please include the `mode_numbers`, `polarisation` and `wavelength` (in nm) for each mode at this port.
